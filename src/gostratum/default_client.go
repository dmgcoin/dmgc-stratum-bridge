...
var walletRegex = regexp.MustCompile("dmg:[a-z0-9]+")

func CleanWallet(in string) (string, error) {
	_, err := util.DecodeAddress(in, util.Bech32PrefixDmgcoinTest)
	if err == nil {
		return in, nil // good to go
	}
	if !strings.HasPrefix(in, "dmg:") {
		return CleanWallet("dmg:" + in)
	}

	// has dmgtest: prefix but other weirdness somewhere
	if walletRegex.MatchString(in) {
		return in[0:65], nil
	}
	return "", errors.New("unable to coerce wallet to valid dmg address")
}
